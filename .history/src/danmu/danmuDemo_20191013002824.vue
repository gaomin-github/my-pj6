    ...
    methods: {
        ...
        channelCheck(danmu, poolIndex) {
            let channels = this.pools[poolIndex].filter(channel => {
                return channel.barrage.startTime+Duration<=danmu.startTime+Duration*playerWidth/(danmu.width+playerWidth)
            });
            let channelIndex=channels[0].index;
            while(channelIndex<=channels[channels.length-1].index){
