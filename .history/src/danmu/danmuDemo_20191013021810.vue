  ...
  export default {
    data: function() {
      return {
        ...
        animationTestStartTiem: 0,
        animationTestRemainTiem: 0,
        // 弹幕存储相关
        danmuQueue: [], //待展示弹幕
    ...
    methods: {
      ...
      // 动画效果测试
      displayAnimation() {
        this.animationTestStartTiem = new Date().getTime();
        this.animationTestRemainTiem = Duration;
      ...
      pauseAnimation() {
        this.animationTestRemainTiem =
          Duration - (new Date().getTime() - this.animationTestStartTiem);
        let animationDom = this.$refs.animationItem;
        animationDom.style.transform = `translateX(-100px)`;
        animationDom.style.transition = ``;
      },
      continueAnimation() {
        let animationDom = this.$refs.animationItem;
        animationDom.style.left = `100px`;
        animationDom.style.transition = ``;
      },
      // 轨道初始化
      initChannels() {
