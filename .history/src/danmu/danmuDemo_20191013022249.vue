    ...
    methods: {
      ...
      pauseAnimation() {
        this.animationTestRemainTime =
          Duration - (new Date().getTime() - this.animationTestStartTime);

        let animationDom = this.$refs.animationItem;
        animationDom.style.transform = `translateX(-${(playerWidth+this.animationTestText.length*16)*(Duration-this.animationTestRemainTime)/Duration)}px)`;
