...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pauseDanmu(){
        this.pools.map(pool=>{
            pool.danmus&&pool.danmus.map(danmu=>{
                ...
                this.$refs.[danmu.index][0].style.left=`${playerWidth-(playerWidth+danmu.text.length*danmu.fontSize)*(Duration-danmu.remainTime)/Duration}px`;
      ...
      continueDanmu(){
        this.pools.map(pool=>{
            pool.danmus&&pool.danmus.map(danmu=>{
                danmu.remainTime=Duration-(this.displayMills-danmu.startTime);
                this.$refs.[danmu.index][0].style.transition=`transform ${danmu.remainTime}ms linear 0s`;
                this.$refs.[danmu.index][0].style.transform=`translateX(-${(playerWidth+danmu.fontSize*danmu.text.length)*danmu.remainTime/Duration}px)`;
                this.$refs.[danmu.index][0].style.left=`${playerWidth-(playerWidth+danmu.text.length*danmu.fontSize)*(Duration-danmu.remainTime)/Duration}px`;
