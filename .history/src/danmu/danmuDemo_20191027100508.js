  ...
  methods: {
    ...
    manualAddDanmu() {
      ...
      let danmu = {
        ...
        startTime:
          this.displayMills +
          (new Date().getTime() - this.lastDisplayTime.getTime()) +
    ...
    filterDanmu() {
      ...
      new Promise((resolve, reject) => {
        ...
        while (i < this.danmuQueue.length) {
          ...
          // 展示弹幕
          this.danmuQueue[i].duration =
            Duration +
            (this.danmuQueue[i].startTime -
              this.displayMills -
              (new Date().getTime() - this.lastDisplayTime.getTime()));

          this.danmuQueue[i].createTime =
            this.displayMills +
            (new Date().getTime() - this.lastDisplayTime.getTime());
    ...
    displayDanmu(danmu, poolIndex) {
      ...
      if (channelResult) {
        ...
        this.$nextTick(() => {
          ...
          let animateWidth =
            Math.ceil(playerWidth + danmu.width) % 2 == 0
              ? Math.ceil(playerWidth + danmu.width)
              : Math.ceil(playerWidth + danmu.width) + 1;
          setTimeout(() => {
      ...
      this.pools.map(pool => {
        pool.danmus &&
          pool.danmus.map(danmu => {
            ...
            danmu.moveTime +=
              new Date().getTime() - danmu.lastDisplayTime.getTime();
            this.$refs[danmu.index][0].style.left = `${playerWidth -
              ((playerWidth + danmu.width) * danmu.moveTime) /
                danmu.duration}px`;
          });
      });
    },
      ...
      this.pools.map(pool => {
        pool.danmus &&
          pool.danmus.map(danmu => {
            ...
            this.$refs[
              danmu.index
            ][0].style.transition = `transform ${danmu.duration -
              danmu.moveTime}ms linear 0s`;
            let animateWidth =
              Math.ceil(
                ((playerWidth + danmu.width) *
                  (danmu.duration - danmu.moveTime)) /
                  danmu.duration
              ) %
                2 ==
              0
    ...
    channelCheck(danmu, pool) {
      ...
      let channels = pool.channels.filter(channel => {
        ...
        if (
          this.displayMills +
            (new Date().getTime() - this.lastDisplayTime.getTime()) -
            channel.danmu.createTime >
          channel.danmu.duration
        ) {
          return true;
        }
        let channelRight =
          ((channel.danmu.width + playerWidth) *
            (this.displayMills +
              (new Date().getTime() - this.lastDisplayTime.getTime()) -
              channel.danmu.createTime)) /
          channel.danmu.duration;
